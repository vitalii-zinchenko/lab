/**
 * get-usage-stats — read load against GET /usage/stats (daily aggregates).
 *
 ...
 *
 * Build & run:
 *   cd perf/k6 && npm run get-usage-stats
 *   BASE_URL=http://staging:8080 npm run get-usage-stats
 *
 * Generate credentials first:
 *   make mock-create-api-keys
 */
import { check } from 'k6';
import { UserCredential, getOrRefreshToken, getUsageStats } from './lib/api';
...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_duration{endpoint:get_usage_stats}': ['p(95)<500'],
  },
};
...
export default function (): void {
  ...
  const res = getUsageStats(from, to, token);
  check(res, { 'get_usage_stats: 200': (r) => r.status === 200 });
}
